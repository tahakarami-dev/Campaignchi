 ...
 *
 * Fixes applied:
 *  - ajaxUrl and nonce moved inside apiFetch() (read at call time, not load time)
 *  - showToast() hardened with typeof check + retry fallback
 *  - <script> tag removed from CampaignsPage — now loaded via PanelLayout
 */
...
(function () {
  ...
  /**
   * AJAX helper — reads CMC_DATA at call time (not at script load time).
   * This ensures CMC_DATA is always available when the request fires.
   */
  async function apiFetch(action, params = {}, method = "GET") {
    // Read at call time — CMC_DATA is guaranteed to exist by now
  ...
  /**
   * Show toast — safe version.
   * Retries after 500ms if panel.js hasn't loaded CMC yet.
   */
  function showToast(msg, type = "success") {
    if (typeof CMC !== "undefined" && typeof CMC.toast === "function") {
      CMC.toast(msg, type);
    } else {
      // Fallback: retry after panel.js loads
      setTimeout(() => {
        if (typeof CMC !== "undefined" && typeof CMC.toast === "function") {
          CMC.toast(msg, type);
        }
  ...
  function renderPickerPanel(mode) {
    ...
    if (mode === "category") {
      renderTermChips(
        ...
        (id, selected) =>
          toggleTermSelection(id, state.selectedCategories, selected)
      );
    }
    if (mode === "tag") {
      renderTermChips(
  ...
  function updateSummary() {
    ...
    if (sumDiscount)
      sumDiscount.textContent = discountVal
  ...
  async function loadEditData() {
    ...
    try {
      ...
      if ($("cmc-field-starts-at"))
        $("cmc-field-starts-at").value = c.starts_at?.replace(" ", "T") ?? "";
      if ($("cmc-field-ends-at"))
        $("cmc-field-ends-at").value = c.ends_at?.replace(" ", "T") ?? "";
      if ($("cmc-field-status"))
